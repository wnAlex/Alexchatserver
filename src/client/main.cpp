...
/*
    当用户登录后执行loginout后再次登录时，主线程和子线程都会卡在rec函数上，所以需要让负责读的子线程一开始就启动
    同时，当主线程发送登录消息后，需要等待子线程收到服务器返回信息并判断是否登录成功后再继续执行
    故需要主线程发完消息后等待子线程相应，需要引入sem，而且需要根据相应结果去执行后续逻辑，登陆成功主线程显示用户命令界面，登陆失败返回主界面
    故需要引入全局变量来表示登录成功与否
*/
//用于读写线程之间的通信的信号量
sem_t rwsem;
...
int main(int argc, const char **argv)
{
    ...
    //初始化读写线程通信用的信号量
    sem_init(&rwsem, 0, 0); //信号量地址，进程or线程，初始资源计数
    ...
    for (;;)
    {
        ...
        */
        ...
        case 1: //登陆业务
        {
            ...
            sem_wait(&rwsem); //主线程send后等待信号量，由子线程处理完登陆的响应消息后，通知这里
        ...
        case 2: //register业务
        {
            ...
            sem_wait(&rwsem); //主线程send后等待信号量，由子线程处理完注册的响应后唤醒
        }
        break;
        case 3: //quit业务
        {
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        else if (LOGIN_MSG_ACK == msgtype)
        {
            ...
            sem_post(&rwsem);//子线程通知主线程登录结果处理完成
            continue;
        }else if(REG_MSG_ACK==msgtype){
            doRegResponse(js);
            sem_post(&rwsem);//子线程通知主线程注册结果处理完成
            continue;
        }
    }
}
